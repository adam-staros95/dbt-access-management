{% macro redshift__configure_data_masking(temp_data_masking_config_table_name, config_data_masking_table_name, create_temp_data_masking_config_table_query, create_data_masking_config_table_query) %}
...
{% macro get_currently_applied_masking_configs_for_objects_from_new_config(temp_data_masking_config_table_name) %}
    {% set query_temp_config_table %}
        select schema_name, alias from access_management.{{temp_data_masking_config_table_name}};
    {% endset %}
    ...
    {% for row in query_temp_config_table_result.rows %}
        {% do temp_config_schema_table_conditions.append(row.schema_name ~ '.' ~ row.alias) %}
    {% endfor %}
...
{% macro get_new_masking_configs(temp_data_masking_config_table_name, objects_in_database) %}
    {% set query_config_table %}
        select t.schema_name, t.alias, c.column_name, c.users_with_access, c.roles_with_access
        from access_management.{{ temp_data_masking_config_table_name }} as t, t.masking_config as c
        where t.database_name || '.' || t.schema_name || '.' || t.alias || '.' ||  case
...
{% macro get_detach_policies_query(policies_to_detach) %}
    ...
    detach masking policy {{policy_to_detach['policy_name']}} on {{ policy_to_detach['schema_name'] }}.{{ policy_to_detach['alias'] }} ("{{ policy_to_detach['column_name'] }}")
...
{% macro get_table_column_types(policies_to_attach) %}
    ...
    {% for policy in policies_to_attach %}
        {% do schema_table_conditions.append(policy['schema_name'] ~ "." ~ policy['alias']) %}
    {% endfor %}
    ...
    {%- for policy_to_attach in policies_to_attach -%}
        ...
        {%- for table_column_type in table_column_types -%}
            {% if policy_to_attach['schema_name'] == table_column_type['schema_name'] and
                  policy_to_attach['alias'] == table_column_type['alias'] and
                ...
                {%- if masking_policies['masking_policy'] is not none and masking_policies['unmasking_policy'] is not none -%}
                    {%- if policy_to_attach['grantee_type'] == 'public' -%}
                        ...
                        ON {{ policy_to_attach['schema_name'] }}.{{ policy_to_attach['alias'] }}("{{ policy_to_attach['column_name'] }}")
                        TO PUBLIC;
        ...
        {% if ns.data_type is none %}
            {% do issues.append(policy_to_attach['schema_name'] ~ '.' ~ policy_to_attach['alias'] ~ '.' ~ policy_to_attach['column_name']) %}
        {% endif %}
    {%- endfor -%}
    {%- endset -%}
