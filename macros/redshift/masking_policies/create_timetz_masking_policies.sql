{% macro create_timetz_masking_policies(currently_existing_project_related_masking_policies) %}

    {% set policy_names = dbt_access_management.get_policy_names_constants() %}
    {% set masking_policy_name = policy_names.MASK_TIMETZ %}
    {% set unmasking_policy_name = policy_names.UNMASK_TIMETZ %}

    {% set create_masking_policy_query %}
    CREATE MASKING POLICY {{masking_policy_name}}
    WITH (_input timetz)
    USING ('00:00:00'::timetz);
    {% endset %}
