{% macro create_smallint_masking_policies(currently_existing_project_related_masking_policies) %}

    {% set policy_names = dbt_access_management.get_policy_names_constants() %}
    {% set masking_policy_name = policy_names.MASK_SMALLINT %}
    {% set unmasking_policy_name =policy_names.UNMASK_SMALLINT %}

    {% set create_masking_policy_query %}
    CREATE MASKING POLICY {{masking_policy_name}}
    WITH (_input smallint)
    USING (-999::smallint);
    {% endset %}
