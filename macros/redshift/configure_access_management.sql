{% macro redshift__configure_access_management(temp_access_management_config_table_name, config_access_management_table_name, create_temp_access_management_config_table_query, create_access_management_config_table_query) %}
...
{% macro get_grants_and_revokes(objects_in_database, config_access_management_table_name, database_identities, should_check_if_config_table_exits=True) %}
    ...
    {% if database_identities | length > 0 %}
        ...
        WHERE database_name || '.' || schema_name || '.' || alias || '.' ||  CASE
