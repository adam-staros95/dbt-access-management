{% macro create_char_masking_policies(currently_existing_project_related_masking_policies) %}

    {% set policy_names = dbt_access_management.get_policy_names_constants() %}
    {% set masking_policy_name = policy_names.MASK_CHAR %}
    {% set unmasking_policy_name =policy_names.UNMASK_CHAR %}
