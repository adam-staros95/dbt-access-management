{% macro create_bigint_masking_policies(currently_existing_project_related_masking_policies) %}

    {% set policy_names = dbt_access_management.get_policy_names_constants() %}
    {% set masking_policy_name = policy_names.MASK_BIGINT %}
    {% set unmasking_policy_name =policy_names.UNMASK_BIGINT %}

    {% set create_masking_policy_query %}
    CREATE MASKING POLICY {{masking_policy_name}}
    WITH (_input bigint)
    USING (FNV_HASH(_input));
    {% endset %}
