{% macro databricks__execute_grants(am_database_name, am_schema_name) %}
    {% if execute %}
        {% if config.get('materialized') != 'ephemeral' %}
            {% set principal_does_not_exist_sql_state = '42704' %}
            {% set query_config_table %}
                select grants
                from {{am_database_name}}.{{am_schema_name}}.{{project_name}}_access_management_config
            ...
            {% set unique_grants = all_grants_for_a_model | unique %}
