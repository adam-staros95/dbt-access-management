{% macro validate_configured_entities(config_table_name = 'config', should_stop_execution=True) %}
    {% if execute %}
        ...
        {% for element in config_table_entities | unique %}
            {% if element not in system_tables_entities %} {% do issues.append(element) %} {% endif %}
        {% endfor %}
        ...
        {% if issues | length > 0 %}
            ...
            {% if should_stop_execution %} {{ exceptions.raise_compiler_error(issue_message) }}
